...
- [x] Ported upstream PR #297: custom per-game media (logo, icon, header, capsule, hero) with a Media tab in the container configuration, centralized `GameImageUtils` resolution (custom media → SteamGridDB → Steam), `CustomMediaUtils` with a media-version invalidation flow wired into list/grid cards and app screens, a `LoadingToast` progress/done indicator for image fetching, bounded centered image-loading spinners, and an indeterminate spinner in the loading dialog.
- [x] Extended installer recovery: "Run installer again" action on the discovery-failure dialog (re-snapshots the executable baseline, returns the session to `INSTALLER_RUNNING`, and relaunches the container), and an explicit installer stage label on the boot splash while an installer session runs.
- [x] Fixed stalled prerequisite installs: vcredist 2010+ now runs fully silent (`/quiet /norestart` instead of `/passive`, avoiding the Wine UI-pump hang at "Validating"), and every chained pre-launch command now has a four-minute watchdog that kills common installer processes and advances the chain without marking the step done, so a hung prerequisite can never block game launch forever.
- [x] Made the library quick-actions menu genuinely useful: per-game container settings (opens the full container configuration for the focused game) and a Steam achievements browser (full-screen overlay with loading state) joined the existing play/install, details, library options, search, and add-game actions.
- [x] Fixed the gamepad shared-memory break caused by the application-id change: `EVSHIM_BASE_PATH` is now set explicitly for the guest shim, and all hardcoded `/data/data/app.gamenative` paths (media-converter env vars, default E: drive, DXVK state cache, Wine storage-drive detection) derive from `BuildConfig.APPLICATION_ID` or the runtime files dir; containers with a stale storage-drive path are migrated on next launch.
- [x] Added ISO/disc-image handling as a separate mounted-media workflow: ISO files are accepted by the install picker but never treated as executables — they are extracted via libarchive (ISO9660, path-traversal and size guards), then routed to the existing installer session when a setup program is found on the disc, or registered as a portable game folder.
...
- [x] Add ISO/disc-image handling as a separate mounted-media workflow; do not treat an ISO as an executable. (Extraction-based via libarchive; true loop mounting is not possible unrooted.)
